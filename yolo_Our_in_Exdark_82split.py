...
class YOLO(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image,enhance, crop = False, count = False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        ...
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        font        = ImageFont.truetype(font='model_data/simhei.ttf', size=np.floor(3e-2 * enhance.size[1] + 0.5).astype('int32'))
        # thickness   = int(max((enhance.size[0] + enhance.size[1]) // np.mean(self.input_shape), 1))
        thickness   =5
        #---------------------------------------------------------#
        #   计数
        #---------------------------------------------------------#
        ...
        if crop:
            for i, c in list(enumerate(top_boxes)):
                ...
                bottom  = min(enhance.size[1], np.floor(bottom).astype('int32'))
                right   = min(enhance.size[0], np.floor(right).astype('int32'))
                
                dir_save_path = "img_crop"
                if not os.path.exists(dir_save_path):
                    os.makedirs(dir_save_path)
                crop_image = enhance.crop([left, top, right, bottom])
    ...
    def get_FPS(self, image, test_interval):
        fps_start_time = time.time()  # 记录开始时间
