...
'''
其核心思想是使用slice(切片)技术，对行进行隔行采样，对列进行隔列采样，
最终将采样的结果按照通道进行拼接，从而达到降维提升通道数目的目标，
使用该方式不会存在信息损失，并且减少了计算量提升速度。
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])#3个残差组件
...
class CSPDarknet(nn.Module):
    def __init__(self, base_channels, base_depth, phi, pretrained):
        ...
        #对于l版本：base_channels=64, base_depth=3
        #-----------------------------------------------#
        ...
        self.dark2 = nn.Sequential(
            ...
            Conv(base_channels, base_channels * 2, 3, 2),#3x3卷积+BN+SiLU
